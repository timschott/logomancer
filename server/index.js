...
hbs.registerHelper('getDescription', function (book, author, word, sentence, definition, options) {
    ...
    if (sentence.indexOf(' ') == -1) {
    	highlighted = '<span style= "background-color: #69dcd6">' + sentence.substring(0, sentence.length - 1) + '</span>' + sentence.substring(sentence.length - 1)
    }
    ...
    tmp = '<div class="tab-pane" id ="' + word.toLowerCase() + '" ' + 'role="tabpanel">' +
        ...
        '<p><span class="special-name">' + author + '</span> uses it in ' + '<span class = "special-name">' + book + '</span> as follows: </p>' +
        '<p> ' + 
        highlighted +
        '<br>' + 
        '</p>' + 
